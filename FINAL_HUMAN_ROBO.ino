...
#define LEFT A3            // Right IR sensor connected to analog pin A2 of Arduino Uno:
#define RIGHT A2           // Left IR sensor connected to analog pin A3 of Arduino Uno:
...
if((distance > 1) && (distance < d)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
  ...
  //Move Forward:
  Motor1.setSpeed(255);  //define motor1 speed:
  Motor1.run(FORWARD);   //rotate motor1 clockwise:
  Motor2.setSpeed(255);  //define motor2 speed:
  Motor2.run(FORWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(255);  //define motor3 speed:
  Motor3.run(FORWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(255);  //define motor4 speed:
  Motor4.run(FORWARD);   //rotate motor4 clockwise:
   if(distance > d) {                          //If the condition is 'true' then the statement below will execute:
...
stop();
  ...
  Motor3.run(RELEASE);  //rotate motor3 anticlockwise:
  Motor4.setSpeed(0);  //define motor4 speed:
  Motor4.run(RELEASE);  //rotate motor4 anticlockwise:
  delay(100);
  ...
  Motor1.run(RELEASE);  //rotate motor1 anticlockwise:
  Motor2.setSpeed(0);  //define motor2 speed:
  Motor2.run(RELEASE);  //rotate motor2 anticlockwise:
  Motor3.setSpeed(255);  //define motor3 speed:
  Motor3.run(FORWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(255);  //define motor4 speed:
  Motor4.run(FORWARD);   //rotate motor4 clockwise:
